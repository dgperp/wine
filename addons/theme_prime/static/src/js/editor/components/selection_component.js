...
let SelectionComponent = AbstractComponent.extend(MarkupRecords, {
    ...
    willStart: async function () {
        ...
        if (this.recordsIDs && this.recordsIDs.length) {
            ...
            // remove unnecessary IDs some records might be disappeared.
            this.recordsIDs = _.intersection(this.recordsIDs, _.map(this.records, function (record) { return record.id }));
        } else {
    ...
    /**
     * @public
     * this method is for external components to avoid extra RPC
     */
    getRecordsData: function (recordsIDs) {
        let records = _.map(recordsIDs, (record) => {
            return _.clone(_.findWhere(this.records, { id: record }));
        });
        return _.compact(records); // compact is mandatory in case record is not available now.
    },
    ...
    _makeRecordsSortable: function () {
        this.$('.tp-selected-record-list').nestedSortable({listType: 'ul',protectRoot: true,handle: '.tp-sortable-handle',items: 'li',toleranceElement: '> .row',forcePlaceholderSize: true,opacity: 0.6,tolerance: 'pointer',placeholder: 'tp-drag-placeholder mt-2',maxLevels: 0,expression: '()(.+)',relocate: this._onRearrangeItems.bind(this)});
    },
    /**
     * @private
    ...
    /**
     * @private
     * @param recordsIDs {Array} IDs of records
     */
    ...
    _renderDropdown: function (records) {
        ...
        if (dropdownTemplate) {
            var $menu = $(qweb.render('theme_prime.tp_autocomplete_dropdown_item', {dropdownTemplate: dropdownTemplate, records: data, term: this.$('.tp-search-input').val(), widget:this}));
