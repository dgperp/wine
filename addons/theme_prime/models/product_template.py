...
class ProductTemplate(models.Model):
    ...
    @api.depends_context(
        'warehouse', 'allowed_company_ids'
    )
    def _compute_free_qty(self):
        for product in self:
            product.free_qty = str(sum(product.product_variant_ids.with_context(warehouse=self._context.get('warehouse', False)).mapped('free_qty')))
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if res.get('fetch_fields'):
            res['fetch_fields'].append('qty_available')
            res['fetch_fields'].append('base_unit_name')
            res['fetch_fields'].append('free_qty')
        if res.get('mapping'):
            res['mapping']['qty_available'] = {'name': 'qty_available', 'type': 'html', 'truncate': True}
            res['mapping']['base_unit_name'] = {'name': 'base_unit_name', 'type': 'html', 'truncate': True}
            res['mapping']['free_qty'] = {'name': 'free_qty', 'type': 'html', 'truncate': True}
